...
SELECT b.bookid , CONCAT( f.name, '-', m.firstname ) AS name, b.starttime,
...
SELECT b.bookid , CONCAT( f.name, '-', m.firstname ) AS name, b.starttime,pcost.cost
FROM Bookings b,
Facilities f,
Members m,
...
(SELECT (CASE WHEN b.memid = 0  THEN f.guestcost
        ...
        WHERE b.memid = m.memid
		AND b.facid = f.facid
		AND b.starttime like '%2012-09-14%') AS pcost
